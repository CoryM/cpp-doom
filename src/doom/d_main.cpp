...
void D_DoomMain(void)
{
    ...
    if (p = M_CheckParm("-turbo"); p != c_Arguments::NotFound)
    {
        ...
        if (p < M_GetArgumentCount() - 1)
            scale = M_GetArgumentAsInt(p + 1);
        if (scale < 10)
            scale = 10;
        if (scale > 400)
            scale = 400;
    ...
    if (p != c_Arguments::NotFound)
    {
        ...
        {
            DEH_snprintf(file, sizeof(file), "%s.lmp", M_GetArgument(p + 1));
        }
...
// [crispy] disable meaningless warning, we always use "-merge" anyway
    ...
    if (p != c_Arguments::NotFound)
    {
        ...
        else
        {
            ...
            if (p + 2 < M_GetArgumentCount() && M_GetArgument(p + 2)[0] != '-')
            {
                startmap = M_GetArgument(p + 2)[0] - '0';
            }
            else
            {
                // [crispy] allow second digit without space in between for Doom 1
                startmap = M_GetArgument(p + 1)[1] - '0';
            }
        }
        autostart = true;
