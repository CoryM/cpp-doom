...
static void *DEH_MiscStart([[maybe_unused]] deh_context_t *context, [[maybe_unused]] char *line)
{
    return NULL;
}

static void DEH_MiscParseLine(deh_context_t *context, char *line, [[maybe_unused]] void *tag)
{
    ...
    if (!doomtype::strcasecmp(variable_name, "Monsters Infight"))
    {
        // See notes above.
    ...
    {
        if (!doomtype::strcasecmp(variable_name, misc_settings[i].deh_name))
        {
