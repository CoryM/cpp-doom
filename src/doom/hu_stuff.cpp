...
static auto HU_DrawCrosshair() -> void
{
    ...
    if (weaponinfo[plr->readyweapon].ammo == am_noammo || plr->playerstate != PST_LIVE || globals::doom::automapactive || globals::doom::menuactive || paused || secret_on)
    {
        return;
    }
...
auto HU_Drawer() -> void
{
    ...
    if (screenblocks > CRISPY_HUD + 1 && (!globals::doom::automapactive || crispy->automapoverlay))
    {
        dp_translucent = true;
    }
    ...
    if (crispy->automapstats == WIDGETS_ALWAYS || (globals::doom::automapactive && crispy->automapstats == WIDGETS_AUTOMAP))
    {
        // [crispy] move obtrusive line out of player view
        if (globals::doom::automapactive && (!crispy->automapoverlay || screenblocks < CRISPY_HUD - 1) && !crispy->widescreen)
        {
    ...
    if (crispy->leveltime == WIDGETS_ALWAYS || (globals::doom::automapactive && crispy->leveltime == WIDGETS_AUTOMAP))
    {
        HUlib_drawTextLine(&w_ltime, false);
    }

    if (crispy->playercoords == WIDGETS_ALWAYS || (globals::doom::automapactive && crispy->playercoords == WIDGETS_AUTOMAP))
    {
