    ...
    for (visplane_t *pl = visplanes; pl < lastvisplane; pl++)
    {
        ...
        if (pl->picnum == skyflatnum || pl->picnum & PL_SKYFLAT)
        {
            ...
            for (x = pl->minx; x <= pl->maxx; x++)
            {
                ...
                if (static_cast<unsigned>(dc_yl) <= static_cast<unsigned>(dc_yh)) // [crispy] 32-bit integer math
                {
        ...
        ds_source =
            static_cast<byte *>(swirling ? R_DistortedFlat(lumpnum) : W_CacheLumpNum(lumpnum, PU::STATIC));
