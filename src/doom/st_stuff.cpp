...
void ST_drawWidgets(boolean refresh)
{
    ...
    if (st_crispyhud)
    {
        ...
        if (plyr->readyweapon == wp_fist && plyr->powers[pw_strength])
        {
            ...
            V_DrawPatch(ST_AMMOX - 21 - endian::SHORT(patch->width) / 2 + endian::SHORT(patch->leftoffset),
                179 - endian::SHORT(patch->height) / 2 + endian::SHORT(patch->topoffset),
                patch);
        }
    ...
    {
        V_CopyRect(ST_FX + DELTAWIDTH, 1, st_backing_screen, endian::SHORT(faceback->width), ST_HEIGHT - 1, ST_FX + DELTAWIDTH, ST_Y + 1);
    }
