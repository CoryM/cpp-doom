...
void SB_Init(void)
{
    ...
    {
        PatchLIFEGEM = static_cast<patch_t *>(W_CacheLumpNum(W_GetNumForName(DEH_String("LIFEGEM0")) + consoleplayer, PU_STATIC));
    }
...
void SB_Drawer(void)
{
    ...
    if (CPlayer->powers[pw_flight])
    {
            ...
            || !(CPlayer->powers[pw_flight] & 16))
        {
            ...
            if (CPlayer->mo->flags2 & MF2_FLY)
            {
                ...
                {
                    V_DrawPatch(20, 17, static_cast<patch_t *>(W_CacheLumpNum(spinflylump + 15, PU_CACHE)));
                }
                else
                {
                    V_DrawPatch(20, 17, static_cast<patch_t *>(W_CacheLumpNum(spinflylump + frame, PU_CACHE)));
    ...
    if (CPlayer->powers[pw_weaponlevel2] && !CPlayer->chickenTics)
    {
        ...
        {
            frame = (leveltime / 3) & 15;
            V_DrawPatch(300, 17, static_cast<patch_t *>(W_CacheLumpNum(spinbooklump + frame, PU_CACHE)));
            BorderTopRefresh = true;
            UpdateState |= I_MESSAGES;
        }
        else
