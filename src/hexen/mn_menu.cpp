...
void MN_DrTextAYellow(const char *text, int x, int y)
{
    ...
    while ((c = *text++) != 0)
    {
        ...
        {
            p = cache_lump_num<patch_t *>(FontAYellowBaseLump + c - 33, PU_CACHE);
            V_DrawPatch(x, y, p);
            x += SHORT(p->width) - 1;
        }
    }
...
// Old Gold skull positions: (40, 10) and (232, 10)
    V_DrawPatch(37, 80, cache_lump_num<patch_t *>(MauloBaseLump + (frame + 2) % 7,
                                       PU_CACHE));
    V_DrawPatch(278, 80, cache_lump_num<patch_t *>(MauloBaseLump + frame, PU_CACHE));
}
...
static void DrawClassMenu(void)
{
    ...
    V_DrawPatch(174, 8, cache_lump_name<patch_t *>(boxLumpName[pclass], PU_CACHE));
    V_DrawPatch(174 + 24, 8 + 12,
                cache_lump_num<patch_t *>(W_GetNumForName(walkLumpName[pclass])
...
static void DrawFileSlots(Menu_t * menu)
{
    ...
    for (i = 0; i < 6; i++)
    {
        V_DrawPatch(x, y, cache_lump_name<patch_t *>("M_FSLOT", PU_CACHE));
        if (SlotStatus[i])
        {
...
static void DrawSlider(Menu_t *menu, int item, int width, int slot)
{
    ...
    {
        V_DrawPatch(x2, y, cache_lump_name<patch_t *>(count & 1 ? "M_SLDMD1" : "M_SLDMD2", PU_CACHE));
    }
    V_DrawPatch(x2, y, cache_lump_name<patch_t *>("M_SLDRT", PU_CACHE));
    V_DrawPatch(x + 4 + slot * 8, y + 7,
        cache_lump_name<patch_t *>("M_SLDKB", PU_CACHE));
}
