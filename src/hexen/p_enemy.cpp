...
/* there is no direct path to the player, so pick another direction */
    ...
    {
        for (tdir = DI_EAST; tdir <= DI_SOUTHEAST; tdir=static_cast<dirtype_t>(static_cast<int>(tdir)+1))
        {
            if (tdir != turnaround)
            {
                actor->movedir = tdir;
...
static void DragonSeek(mobj_t * actor, angle_t thresh, angle_t turnMax)
{
    ...
    {                           // attack the destination mobj if it's attackable
        ...
        if (abs(static_cast<int>(actor->angle) - static_cast<int>(R_PointToAngle2(actor->x, actor->y, target->x, target->y))) < static_cast<int>(ANG45 / 2))
        {
    ...
    {                           // Hit the target thing
        if (actor->target && P_Random() < 200)
        {
            ...
            for (i = 0; i < 5; i++)
            {
                ...
                if (abs(static_cast<int>(angleToSpot - angleToTarget)) < static_cast<int>(bestAngle)) {
...
void A_DragonFlight(mobj_t * actor)
{
    ...
    if (actor->target)
    {
        ...
        angle = R_PointToAngle2(actor->x, actor->y, actor->target->x,
                                actor->target->y);
        if (abs(static_cast<int>(actor->angle - angle)) < static_cast<int>(ANG45 / 2)
            && P_CheckMeleeRange(actor))
        {
            P_DamageMobj(actor->target, actor, actor, HITDICE(8));
            S_StartSound(actor, SFX_DRAGON_ATTACK);
        }
        else if (abs(static_cast<int>(actor->angle - angle)) <= static_cast<int>(ANG1 * 20))
        {
            P_SetMobjState(actor, static_cast<statenum_t>(actor->info->missilestate));
            S_StartSound(actor, SFX_DRAGON_ATTACK);
        }
...
void A_DemonAttack2(mobj_t * actor)
{
    ...
    if (auto mo = P_SpawnMissile(actor, actor->target, fireBall); mo)
    {
        mo->z += 30 * FRACUNIT;
...
void A_FiredSpawnRock(mobj_t * actor)
{
    ...
    if (auto mo = P_SpawnMobj(x, y, z, static_cast<mobjtype_t>(rtype)); mo)
    {
        mo->target = actor;
...
void A_IceGuyLook(mobj_t * actor)
{
    ...
    if (P_Random() < 64)
    {
        ...
        P_SpawnMobj(actor->x + FixedMul(dist, finecosine[an]),
                    ...
                    actor->z + 60 * FRACUNIT,
                    static_cast<mobjtype_t>(MT_ICEGUY_WISP1 + (P_Random() & 1)));
    }
}
    ...
    switch (mode)
    {
        ...
        case SORC_STOPPING:    // Balls stopping
            if ((parent->special2.i == actor->type) &&
                ...
                (abs(static_cast<int>(angle - (parent->angle >> ANGLETOFINESHIFT))) < static_cast<int>(30 << 5)))
            {
                // Can stop now
