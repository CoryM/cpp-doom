                ...
                if (BlockingMobj && (BlockingMobj->flags2 & MF2_REFLECTIVE))
                {
                    ...
                    switch (BlockingMobj->type)
                    {
                        ...
                        case MT_CENTAURLEADER:
                            if (abs(static_cast<int>(angle - BlockingMobj->angle)) >> 24 > 45)
                                goto explode;
...
void P_SpawnPlayer(mapthing_t * mthing)
{
    ...
    {
        p->pclass = static_cast<pclass_t>(P_Random() % 3);
        if (p->pclass == PlayerClass[mthing->type - 1])
        {
            p->pclass = static_cast<pclass_t>((p->pclass + 1) % 3);
        }
...
void P_SpawnMapThing(mapthing_t * mthing)
{
    ...
    {
        R_PointInSubsector(mthing->x << FRACBITS, mthing->y << FRACBITS)->sector->seqType = static_cast<seqtype_t>(mthing->type - 1400);
        return;
    }
