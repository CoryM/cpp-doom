...
{
    net_packet_t *packet = zmalloc<net_packet_t *>(sizeof(net_packet_t), PU::STATIC, 0);

    if (initial_size == 0)
        initial_size = 256;

    packet->alloced = initial_size;
    packet->data    = zmalloc<byte *>(initial_size, PU::STATIC, 0);
    packet->len     = 0;
    packet->pos     = 0;
...
static void NET_IncreasePacket(net_packet_t *packet)
{
    ...
    auto *newdata = zmalloc<byte *>(packet->alloced, PU::STATIC, 0);
