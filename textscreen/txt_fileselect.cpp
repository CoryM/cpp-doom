...
static char *SelectDirectory(char *window_title)
{
    ...
    if (pidl != NULL)
    {
        ...
        {
            result = M_StringDuplicate(selected);
        }
...
char *TXT_SelectFile(const char *window_title, const char **extensions)
{
    ...
    argv[0] = M_StringDuplicate(ZENITY_BINARY);
    argv[1] = M_StringDuplicate("--file-selection");
    argc    = 2;
    ...
    if (extensions == TXT_DIRECTORY)
    {
        argv[argc] = M_StringDuplicate("--directory");
        ++argc;
    }
    else if (extensions != NULL)
    {
        ...
        argv[argc] = M_StringDuplicate("--file-filter=*.* | *.*");
        ++argc;
    }
